...
# Read in model csv
reactionlist <- read.csv("model_csv/model.csv") 
...
# datasheet <- subset(read.table("data/data_mark_out.csv",sep=",",header=TRUE), time >0 ) # needs columns condition, name, time, value, sigma, time = 0 were removed due to experimental problems
...
center <- ini
cat("test obj with ini:",obj(ini)$value)
if(do.fit){
  print("start fit")
  ...
  print(plotCombined(prediction,  data) + theme_bw())
  
  print("chi2 per condition:")
  cat(dim(do.call(rbind,data))[1], obj(myfit$argument)$value, length(myfit$argument),"total","\n")
  for(i in conditions){
    cat(dim(data[[i]])[1],obj(myfit$argument, conditions = i)$value,i,"\n")
  }
  
  plotCombined(prediction,  data, grepl("betacar",condition) & name %in% c("obcar", "ob10")) + xlab("time[min]") + ylab("concentration [nM]")
}
