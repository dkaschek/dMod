...
def DetermineGraphStructure(SM, F, X):
    graph={}    
...
def GetBestPair(cycle, SM, fluxpars, X, LCLs):    
    for state in cycle:
        for LCL in LCLs:
    ...
    dimList=[]
    for state in cycle:
        dimList.append(GetDimension(state, X, SM))
    ...
    for i in range(len(dimList)):
        if(dimList[i]==minOfDimList):
            dim, sign=GetDimension(cycle[i], X, SM, True)
            appearList=[]
            #print(sign)
            if(sign=="minus"):
                for fp in GetNegFluxParameters(SM, fluxpars, X, cycle[i]):
                    appearList.append(GetAppearances(fp, fluxpars, SM))
            if(sign=="plus"):
                for fp in GetPosFluxParameters(SM, fluxpars, X, cycle[i]):
                    appearList.append(GetAppearances(fp, fluxpars, SM))
            #print(appear)
            #print(minOfDimList)
            if(sum(appearList)<n2beat):
                ...
                if(minOfDimList==1 and max(appearList)==1):
                    besttype=1
                else:
                    if(max(appearList)==1 and minOfDimList>1):
                        besttype=2
                    else:
                        besttype=3
    return(besttype, beststate, bestflux)
...
#### Remove cycles step by step
    ...
    while(cycle!=None):
        ...
        #printmatrix(SM)
        #print(F)
        minType, state2Rem, fp2Rem = GetBestPair(cycle, SM, fluxpars, X, LCLs)
        #print(cycle)
        #print(state2Rem)
        #print(fp2Rem)
        #print(minType)
