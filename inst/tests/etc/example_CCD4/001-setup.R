...
errorModel <- setNames(paste0("sqrt(",observables, "^2 * srel_", observables, "^2 + sabs_", observables, "^2)"), names(observables))
e <- Y(errorModel, g)
...
# Read fit
errpars <- grep("srel|sabs", getParameters(e), value = TRUE)
errpars <- setNames(seq(0,10,length.out = length(errpars)), errpars)
ini <- as.parvec(readRDS(system.file("examples/example_CCD4/mstrust_CCD4.Rds", package = "dMod"))) 
ini <- c(ini, errpars)
# Prediction  
