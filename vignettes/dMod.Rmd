...
---
fr_dk846@bwforcluster.bwservices.uni-heidelberg.de
...
out <- runbg_bwfor({
  mstrust(obj + constr, center = myfit$argument, fits = 20, cores = 1, sd = 1, samplefun = "rnorm", fixed = fixed, conditions = "Epo")
}, machine = "fr_dk846@bwforcluster.bwservices.uni-heidelberg.de", filename = "out_dMod", cores = 16, nodes = 1, walltime = "00:00:02:00", compile = FALSE)
...
In our case we find several local optima in the proximity of the original fit. Solutions for a parameter frame can be plotted by the `plotArray()` function. To see both, internal states and observables, the controls of `g` are changed via the `controls()` function.
