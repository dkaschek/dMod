...
model0 <- odemodel(reactions, modelname = "jak-stat", compile = TRUE, 
                   fixed = fixed.zero, jacobian = "jacvec.lsodes")
```
...
x <- Xs(model0, optionsSens = list(method = "lsodes", lrw = 2118))
...
fitlist <- mstrust(obj + constr, center = mu, fits = 20, cores = 4, lower = -1, upper = 1, samplefun = "runif")
pars <- as.parframe(fitlist)
plotValues(subset(pars, converged))

```

In our case we find two local optima. Solutions for a parameter frame can be plotted by the `plotArray()` function. To see both, internal states and observables, the controls of `g` are changed via the `controls()` function.
