...
data <- do.call(rbind, lapply(1:10, function(i) {
  ...
  (x*p)(timesD, ini, deriv = FALSE, conditions = paste0("subject_", i)) %>% 
    as.data.frame() %>% 
    mutate(sigma = 0.02) %>%
...
data %>% as.datalist(split.by = "condition") %>% plot()

model <- modelNLME(x*p)

fit <- nlme::nlme(value~model(time, name, A, kA, kB), data = data,
                  fixed = A+kA+kB ~ 1, random = A+kA ~ 1 | condition, 
                  weights = varIdent(form = ~ 1 | name),
                  start = ini)


prediction <- fit %>% coef() %>% split(f = 1:10) %>% 
  lapply(function(pars) (x*p)(times, unlist(pars))[[1]]) %>%
  as.prdlist(names = paste0("subject_", 1:10))
