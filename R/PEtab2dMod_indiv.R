...
# 
# # .. Symbolic Trafo -----
# trafo <- setNames(nm = unique(c(getParameters(myreactions), getParameters(myg), getSymbols(myerrors))))
...
# prd0 <- PRD_indiv(prd0 = Reduce("*", fns), est.grid = gridlist$est.grid, fix.grid = fix.grid)
# obj_data <- normL2_indiv(data, prd0,errmodel = myerr, 
#                          est.grid = gridlist$est.grid, fix.grid = fix.grid, 
#                          times = seq(0,max(as.data.frame(mydata)$time), len=501))
# # .. Collect final list -----
# petab <- list(
#   symbolicEquations = symbolicEquations, 
#   odemodel = myodemodel, 
#   # [ ] Vollständige Datenspezifizierung: data, gridlist, myerr
#   data = mydata, 
#   gridlist = gridlist,
#   e = myerr, 
#   fns = fns,
