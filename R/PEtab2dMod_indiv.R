...
# .. Initialize fix.grid, est.grid and trafo -----
# Take unique identifying column only
cg <- data.table(mycondition.grid)
cg <- cg[,!"conditionName"]
setnames(cg, "conditionId", "condition")
# Determine which columns contain values and parameter names
...
is_mixed <- intersect(is_string, is_numeric) # possible
...
# [ ] Need example for preeqEvents. Then move trafo below
...
# .... 1 ## Specify prediction functions ------
# [ ] Pre-equilibration
...
myouterpars <- getParameters(gridlist$est.grid)
...
petab <- list(reactions = myreactions, 
